-- Gui2Lua implemented directly in lua
-- github.com/MalwareMania
...
local function Dump(root, rootPE)
	assert(typeof(root) == "Instance", "Gui2Lua.Dump expects an Instance as the first argument")
...
return {
	...
	SetOptions = function(opts)
		if opts.includeAttributes ~= nil then INCLUDE_ATTRIBUTES = opts.includeAttributes end
		if opts.includeTags       ~= nil then INCLUDE_TAGS       = opts.includeTags       end
		if opts.includePivot      ~= nil then INCLUDE_PIVOT      = opts.includePivot      end
		if opts.skipDefaults      ~= nil then SKIP_DEFAULT_VALUES = opts.skipDefaults      end
	end,
	AddExtraProperties = function(className, props)
		ExtraProperties[className] = ExtraProperties[className] or {}
		for _, p in ipairs(props) do
			table.insert(ExtraProperties[className], p)
		end
	end,
}
